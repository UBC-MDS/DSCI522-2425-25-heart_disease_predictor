...
# Test for missing diagnosis column
def test_main_missing_column(tmp_path):
    csv_path = tmp_path / "missing_column_data.csv"
    output_dir = tmp_path / "output"
    missing_column_data.to_csv(csv_path, index=False)
    
    with pytest.raises(KeyError, match="'diagnosis' column not found in the dataset."):
        main(csv_path, output_dir)
