...
To run the analysis, you'll be using Docker to set up a containerized environment:

1. **Reset the project to a clean state** (removes any files generated by previous runs):

    ```bash
    docker-compose run --rm analysis-env make clean
    ```

2. **Run the analysis** in its entirety:

    ```bash
    docker-compose run --rm analysis-env make all
    ```

This will execute the analysis from start to finish, ensuring all necessary steps are followed in a clean container environment.

## Dependencies

Docker is used to manage the software dependencies for this project. The Docker image used for this project is based on `quay.io/jupyter/minimal-notebook:afe30f0c9ad8`. Additional dependencies are specified in the `Dockerfile`.
...
1. Modify the `Dockerfile` to install the new dependency.
2. Rebuild the Docker image:

    ```bash
    docker-compose build analysis-env
    ```

3. Re-run the analysis using the updated container image.
